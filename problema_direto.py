...
def problema_adjunto(L1_0, L2_0, T1, T2, Y1, Y2):
    t = t_inicial
    times = [t]
    L1hist = [L1_0]
    L2hist = [L2_0]
...
# ----------
def problema_sensibilidade(DT1, DT2, Du, T2):
    t = t_inicial
    times = [t]
    DT1hist = [DT1]
    DT2hist = [DT2]
    for i in range(N):
        DT1, DT2 = rk4_step_sensibilidade(DT1, DT2, Du[i], T2[i])
        t += h
        times.append(t)
        DT1hist.append(DT1)
        DT2hist.append(DT2)
    return np.array(times), np.array(DT1hist), np.array(DT2hist)

# -- funções helper problema de sensibilidade-- #

def rk4_step_sensibilidade(tn, DT1, DT2, Y1, T1, Y2, T2):
    # k1
    k11 = dDT1(DT1, DT2, Y1, T1)
    k12 = dDT2(DT1, DT2, Y2, T2)
    
    # k2 (t + h/2)
    DT1_k2 = DT1 + 0.5*h*k11
    DT2_k2 = DT2 + 0.5*h*k12
    k21 = dDT1(tn + 0.5*h, DT1_k2, DT2_k2)
    k22 = dDT2(tn + 0.5*h, DT1_k2, DT2_k2)

    # k3 (t + h/2)
    DT1_k3 = DT1 + 0.5*h*k21
    DT2_k3 = DT2 + 0.5*h*k22
    k31 = dDT1(tn + 0.5*h, DT1_k3, DT2_k3)
    k32 = dDT2(tn + 0.5*h, DT1_k3, DT2_k3)

    # k4 (t + h)
    DT1_k4 = DT1 + h*k31
    DT2_k4 = DT2 + h*k32
    k41 = dDT1(tn + h, DT1_k4, DT2_k4)
    k42 = dDT2(tn + h, DT1_k4, DT2_k4)
